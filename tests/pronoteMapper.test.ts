...
} from '../src/pronote/mapper';
import { PawnoteDeadline, PawnoteGrade, PawnoteHomework, PawnoteLesson, PawnoteTimetableEntry } from '../src/pronote/types';

describe('Pronote mapper', () => {
  it('maps homework to task with defaults', () => {
    const homework: PawnoteHomework = {
      id: 'hw-1',
      subject: 'Biology',
    ...
    const task = mapHomeworkToTask(homework, { defaultTimeEstimateMinutes: 50 });
  ...
  it('maps deadlines with metadata', () => {
    const deadline: PawnoteDeadline = {
      id: 'dl-1',
    ...
    const record = mapDeadlineToRecord(deadline);
  ...
  it('maps grades correctly', () => {
    ...
    const record = mapGradeToRecord(grade);
    expect(record).toEqual({
      id: 'gr-1',
      source: 'pronote',
      sourceId: 'gr-1',
  ...
  it('maps lessons and timetable entries', () => {
    ...
    const lessonRecord = mapLessonToRecord(lesson);
    ...
    const entry: PawnoteTimetableEntry = {
      id: 'tt-1',
      title: 'Chemistry',
      day: '2024-02-03',
      startTime: '09:00',
    ...
    const timetableRecord = mapTimetableEntryToRecord(entry);
