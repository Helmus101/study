...
class AIService {
    ...
    async validateAcademicContent(content) {
...
${request.context?.subjects ? `Current Subjects: ${request.context.subjects.map((s) => s.name).join(', ')}` : ''}
${request.context?.currentTasks ? `Other Current Tasks: ${request.context.currentTasks.map((t) => t.title).join(', ')}` : ''}
...
Respond in JSON format:
{
  ...
  "suggestions": ["Additional tips or considerations"]
    ...
    async generateDailyStudyPlan(request) {
...
${request.context?.subjects ? `All Subjects: ${request.context.subjects.map((s) => s.name).join(', ')}` : ''}
${request.context?.currentTasks ? `Current Tasks: ${request.context.currentTasks.map((t) => `${t.title} (${t.priority})`).join(', ')}` : ''}
${request.context?.upcomingEvents ? `Upcoming Events: ${request.context.upcomingEvents.map((e) => `${e.title} at ${e.startTime}`).join(', ')}` : ''}
...
Respond in JSON format:
{
  ...
  "warnings": ["Potential issues to watch out for"]
    ...
    async getRecommendations(request) {
        const contextInfo = request.context?.subjects?.find((s) => s.id === request.subjectId);
...
${request.context?.subjects ? `Current Subjects: ${request.context.subjects.map((s) => s.name).join(', ')}` : ''}
${request.context?.currentTasks ? `Current Tasks: ${request.context.currentTasks.map((t) => t.title).join(', ')}` : ''}
...
Respond in JSON format:
{
  ...
  "context": "Brief context explaining why these recommendations are relevant"
    ...
    async getStudySessionHints(request) {
        const contextInfo = request.context?.subjects?.find((s) => s.id === request.subjectId);
