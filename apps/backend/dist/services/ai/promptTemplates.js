...
Remember: Provide educational support that enhances learning and understanding.`;
    ...
    static academicValidation(content) {
        return `${this.ACADEMIC_GUARDRAIL}
...
PromptTemplates.ACADEMIC_GUARDRAIL = `
...
`;
PromptTemplates.CONTEXT_PREFIX = (context) => {
    let contextStr = '';
    if (context?.subjects?.length) {
        contextStr += `Student's Subjects: ${context.subjects.map((s) => s.name).join(', ')}\n`;
    }
    if (context?.currentTasks?.length) {
        contextStr += `Current Tasks: ${context.currentTasks.map((t) => `${t.title} (${t.priority})`).join(', ')}\n`;
    }
    if (context?.upcomingEvents?.length) {
        contextStr += `Upcoming Events: ${context.upcomingEvents.map((e) => `${e.title} at ${e.startTime}`).join(', ')}\n`;
    }
    return contextStr;
};
